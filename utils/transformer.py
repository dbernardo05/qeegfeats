...
class Hurst:
    ...
    """
    """
    In regards to following error:
    ../Alex_Gilberto/transformer.py:20: FutureWarning: pd.expanding_std is deprecated for ndarrays and will be removed in a future version
    ... in future, could attempt: pandas.core.window.Expanding.std
    """
    def get_name(self):
        return 'hurst'
    ...
    def apply(self, X):
        def apply_one(x):
            ...
            s = pd.Series(x).expanding().std().values[1:]
            ...
            m, b = np.linalg.lstsq(x_axis, y_axis, rcond=-1)[0]
            return m
    ...
    def hfd(self, X):
        ...
        for k in range(2, self.kmax + 1):
            ...
            for m in range(1, k + 1):
                i_end = (N - m) / k # int
                inds = np.arange(m - 1, m + (i_end + 1) * k - 2, k).astype(int)
                Lmk_sum = np.sum(abs(np.diff(X[inds], n=1)))
